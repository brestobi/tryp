...
serve(async (request: Request) => {
  ...
  try {
    ...
    const { data: ride, error: rideError } = await adminClient
      .from("rides")
      .select("id, passenger_id, fare, payment_method, payment_status, payment_reference, ride_reference")
    ...
    const rideReference = String(ride.ride_reference ?? "").trim();
    const previousReference = String(ride.payment_reference ?? "").trim();
    if (!/^T-[0-9]{5,}(?:-[0-9]+)?$/.test(rideReference)) {
      return response({ error: "Ride does not have a valid friendly payment reference." }, 422);
    }

    // Paystack references are globally unique. The first attempt uses the
    // customer-facing ride reference; subsequent attempts add a readable
    // attempt suffix (T-00001-2, T-00001-3, ...).
    let reference = rideReference;
    if (previousReference) {
      const previousMatch = previousReference.match(/^T-([0-9]{5,})(?:-([0-9]+))?$/);
      if (previousMatch) {
        const previousAttempt = previousMatch[2] ? Number(previousMatch[2]) : 1;
        reference = `T-${previousMatch[1]}-${previousAttempt + 1}`;
      }
    }
