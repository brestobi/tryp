...
const FROM_EMAIL = "norepy@updates.illdoit.space";
  ...
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="padding:32px 12px;background:#f4f4f5;"><tr><td align="center">
    <table role="presentation" width="600" cellspacing="0" cellpadding="0" style="max-width:600px;width:100%;background:#fff;border-radius:20px;overflow:hidden;">
      ...
      <tr><td style="padding:28px 32px;">
        ...
        <p style="margin:24px 0 0;color:#777;font-size:12px;line-height:1.6;">Keep this email for your records. For questions, contact support@tryp.app and include the invoice number above.</p>
      </td></tr>
    </table>
  ...
  try {
    ...
    if (existing) {
      ...
      if (existing.status === "sent") {
        return new Response(JSON.stringify({ skipped: true, status: "sent" }), { status: 200 });
      }
    ...
    if (!claimed) return new Response(JSON.stringify({ skipped: true, reason: "Already claimed" }), { status: 200 });
