-- TRYP push delivery credential hardening
--
-- The earlier push migration used ALTER DATABASE custom settings. Hosted
-- Supabase does not allow those settings for this project, so the trigger now
-- reads both credentials from Supabase Vault.
--
-- The following Vault secret names must be populated after this migration:
--   tryp_push_supabase_service_key
--   tryp_push_webhook_secret
--
-- The values are intentionally not stored in this migration.
...
COMMENT ON FUNCTION public.trigger_push_on_notification() IS
  'Forwards notification rows to FCM through the Edge Function using credentials stored in Supabase Vault.';
