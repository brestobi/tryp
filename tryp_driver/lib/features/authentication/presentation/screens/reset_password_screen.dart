...
class _ResetPasswordScreenPageState
  ...
  void _cancel() {
    ...
    if (widget.onCancel == null && mounted) context.go(Routes.login);
  }
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      // End the recovery session so the driver signs in normally next time.
      // A sign-out failure must not hide a successful password update.
      try {
  ...
  Widget _buildSuccess() {
    return Column(
      ...
      children: [
        ...
        Text(
          'Your driver account is secure. Sign in with your new password.',
          style: TRYPTypography.bodyLarge.copyWith(color: TRYPColors.grey),
          textAlign: TextAlign.center,
        ),
        const SizedBox(height: 44),
        PrimaryButton(label: 'Go to Login', onPressed: _cancel),
      ],
    );
  }
}
